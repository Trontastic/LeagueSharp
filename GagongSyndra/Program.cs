...
namespace GagongSyndra
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            TargetSelector.AddToMenu(Menu.SubMenu("SimpleTs"));
        ...
        {   
            ...
            //R, Ignite 
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.Team != Player.Team && enemy.Distance(Player.Position) <= R.Range && !enemy.IsDead))
                if (buffCheck(enemy))
                {
                    //R
